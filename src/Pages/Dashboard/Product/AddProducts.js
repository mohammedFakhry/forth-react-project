...
const AddProducts = () => {
    ...
    const dummyForm = { category: null, title: 'dummy', description: 'dummy', price: 222, discount: 0, About: 'About', Stock: 0 }
    ...
    async function handleEdit(e) {
        ...
        try {
            const res = await Axios.post(`${PRODUCT}/edit/${productId}`, form)
    ...
    async function handleImagesChange(e) {
        ...
        for (let i = 0; i < imagesAsFiles.length; i++) {
            ...
            imagesData.append('product_id', productId)
            try {
        ...
        <div key={idx} className='border w-100 p-2'>
            ...
            <div className='custem-progress mt-3 w-100'>
                <span className='inner-progress'
                    ...
                    // percent={`${progressImages[idx]}%`}
                    // style={{ width: `${progressImages[idx]}%` }}
                >
                </span>
            </div>
        </div>
    ...
    return (
        <Form className='bg-white w-100 mx-2 p-3' onSubmit={handleEdit}>
            ...
            <Form.Group className='mb-3' controlId='exampleForm.ControlInput1'>
                ...
                <Form.Control name='title' value={form.title} type='text' placeholder='title...' onChange={handleChange} disabled={!isSend} required />
            </Form.Group>
            <Form.Group className='mb-3' controlId='exampleForm.ControlInput2'>
                <Form.Label>Description</Form.Label>
                <Form.Control name='description' value={form.description} type='text' placeholder='description...' onChange={handleChange} disabled={!isSend} required />
            </Form.Group>
            <Form.Group className='mb-3' controlId='exampleForm.ControlInput3'>
                <Form.Label>Price</Form.Label>
                <Form.Control name='price' value={form.price} type='number' placeholder='price...' onChange={handleChange} disabled={!isSend} required />
            </Form.Group>
            <Form.Group className='mb-3' controlId='exampleForm.ControlInput4'>
                <Form.Label>Discount</Form.Label>
                <Form.Control name='discount' value={form.discount} type='number' placeholder='discount...' onChange={handleChange} disabled={!isSend} required />
            </Form.Group>
            <Form.Group className='mb-3' controlId='exampleForm.ControlInput5'>
                <Form.Label>About</Form.Label>
                <Form.Control name='About' value={form.About} type='text' placeholder='about...' onChange={handleChange} disabled={!isSend} required />
            </Form.Group>
            <Form.Group className='mb-3' controlId='exampleForm.ControlInput6'>
                <Form.Label>Stock</Form.Label>
                <Form.Control name='Stock' value={form.Stock} type='number' placeholder='stock...' onChange={handleChange} disabled={!isSend} required />
            </Form.Group>
            ...
            <div onClick={handleOpenUploadImages} className='d-flex align-items-center justify-content-center gap-2 mb-3 py-3 rounded w-100 flex-column' style={{ border: !isSend? '2px dashed gray': '2px dashed #0086fe', cursor: isSend && 'pointer' }}>
